...
var ASM_CONSTS = [function($0, $1) { { var str = UTF8ToString ($0); try { var res = eval (str); if (res === null || res === undefined) return 0; res = res.toString (); setValue ($1, 0, "i32"); } catch (e) { res = e.stack; setValue ($1, 1, "i32"); if (res === null) res = "unknown exception"; } var buff = Module._malloc((res.length + 1) * 2); stringToUTF16 (res, buff, (res.length + 1) * 2); return buff; } },
 function($0, $1, $2) { { try { var funcNameJsString = UTF8ToString ($0); var registeredFunctions = window.__blazorRegisteredFunctions; if (!(registeredFunctions && registeredFunctions.hasOwnProperty(funcNameJsString))) { throw new Error('Could not find registered function with name "' + funcNameJsString + '".'); } var funcInstance = registeredFunctions[funcNameJsString]; return funcInstance.call(null, $1); } catch (ex) { setValue ($2, 1, 'i32'); var exceptionResultString = ex.stack; var exceptionResultBuffer = Module._malloc((exceptionResultString.length + 1) * 2); stringToUTF16 (exceptionResultString, exceptionResultBuffer, (exceptionResultString.length + 1) * 2); return exceptionResultBuffer; } } }];
