...
export function invokeWithJsonMarshallingAsync<T>(identifier: System_String, asyncProtocol: System_String, ...argsJson: System_String[]) {
    const identifierJsString = platform.toJavaScriptString(identifier);
    const funcInstance = getRegisteredFunction(identifierJsString);
    ...
    return result.then(res => invokeDotNetMethod(async.function, async.success, JSON.stringify(res)));
}
