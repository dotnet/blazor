...
");
    ...
    internal class RouteTableEdited
    {
        private string[] _parametersNames;
        public RouteTableEdited(RouteEntry[] routes)
        {
            Routes = routes;
            _parametersNames = routes.SelectMany(r => r.Template.GetParameterSegmentsValues())
                .Distinct()
                .ToArray();
        }
...
");
        ...
        internal void Route(RouteContext routeContext)
        {
            ...
            foreach (var route in Routes)
            {
                ...
                if (routeContext.Handler != null)
                {
                    InitMissingParameters(routeContext);
                    return;
                }
            }
        }

        private void InitMissingParameters(RouteContext routeContext)
        {
            foreach (var parameterName in _parametersNames)
            {
                if (routeContext.Parameters == null)
                    routeContext.Parameters = new Dictionary<string, object>();
                if (!routeContext.Parameters.ContainsKey(parameterName))
                    routeContext.Parameters.Add(parameterName, null);
            }
        }
    }
}
